...
def pca_to_grey(image, mask, inverted=False):
    ...
    # print(np.min(newmat)), print(np.max(newmat))
    # print(np.median(newmat)), print(np.mean(newmat))
    # newpoints = newmat[filter_array > 0]
    # print(np.min(newpoints)), print(np.max(newpoints))
    # print(np.median(newpoints)), print(np.mean(newpoints))
...
if __name__ == "__main__":
    ...
    petal_image = cv2.imread("/Users/erb/Desktop/CS/PigmentSpotting/images/F1P102_Spot_Side2_200723_cropped.jpg")
